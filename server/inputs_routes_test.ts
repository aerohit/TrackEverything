...
Deno.test({
  ...
  async fn() {
    ...
    try {
      ...
      // A voice transcript with no hyphen + lowercase still finds the item.
      const hit = await search("dope max pre workout");
      assertEquals(hit[0]?.name, "Dope-Max Pre-Workout");

      // Partial / single word matches too.
