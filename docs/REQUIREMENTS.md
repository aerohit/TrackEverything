...
| 2026-06-16 | Added R-RT-7 + ADR-023: **"Ask LLM"** screen (`/ask`) — preset + free-text questions (typed or OS-keyboard-dictated) answered by Claude over the **last 48h** of check-ins + intake. New `POST /api/ask` gathers the data server-side and an SDK-isolated `Advisor` seam (pure prompt builder `server/advise.ts`, `AnthropicAdvisor`) calls the model; optional (503 without `ANTHROPIC_API_KEY`, which prod already has). Server (prompt-builder + route w/ mock advisor) + web (`askLlm`) tests; Postman updated; screen + 503 + answer render browser-verified. |
