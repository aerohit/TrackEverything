...
| R-CAP-18 | **Log capture overhaul (v2):** the Log screen captures an intake in one of three ways — **photo** of a meal/drink, **voice** (transcribed **on-device** via the browser's Web Speech API → recognized), or one tap on a **recent item**. Photo/voice are recognized by Claude into a name + quantity + unit + estimated nutrients, and matched against the catalog; the user reviews a **quick-confirm** (editable name/qty/unit/time) and either logs against a matched item, **saves it as a new item** (full recognized draft) and logs, or logs by name. The freeform manual entry form is removed. Anthropic is the only API key — recognition 503s with a friendly message when unset, and voice needs no server key (Firefox lacks Web Speech → photo/recent still work). ([ADR-020](ARCHITECTURE.md#adr-020)) | Built |
