...
> stays current. **Last updated:** 2026-06-15 (R-CAP-18/19 + ADR-020: Log capture
> overhaul — photo / voice / recent, recognize + match, quick-confirm) **Owner:** aerohit
...
| R-CAP-18 | **Log capture overhaul (v2):** the Log screen captures an intake in one of three ways — **photo** of a meal/drink, **voice** (recorded → transcribed → recognized), or one tap on a **recent item**. Photo/voice are recognized by Claude into a name + quantity + unit + estimated nutrients, and matched against the catalog; the user reviews a **quick-confirm** (editable name/qty/unit/time) and either logs against a matched item, **saves it as a new item** (full recognized draft) and logs, or logs by name. The freeform manual entry form is removed. Photo/voice are optional — each 503s with a friendly message when its key is unset. ([ADR-020](ARCHITECTURE.md#adr-020)) | Built |
...
| 2026-06-15 | v2-2f (Log capture overhaul, in review): added R-CAP-18/19 + ADR-020; R-CAP-16 → Built. The Log screen replaces the freeform manual form with three capture modes — **photo**, **voice** (`POST /api/transcribe`, OpenAI-compatible Whisper behind a `Transcriber` seam), and **recent items** (`GET /api/intake/recent-items`). Photo/phrase → `POST /api/intake/recognize` (Claude behind an `IntakeRecognizer` seam) → recognized name/qty/unit + estimated nutrients **matched** against the catalog → a quick-confirm to log against a match, **save as a new item**, or log by name. AI seams optional (503 + graceful UI). Server (parser + transcribe/recognize/recent routes, mocked seams) + web client tests; recent→confirm→log + 503 fallbacks browser-verified; live photo/voice device-verified. |
