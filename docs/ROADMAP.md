...
> **Status:** Living document. **Last updated:** 2026-06-15 (v2-2h: fuzzy item search via pg_trgm — recognized/typed names match stored items despite punctuation/mishears)
...
| 2026-06-15 | Slice v2-2h (fuzzy item search, in review): ADR-022. `listItems` search moves from strict `ILIKE` to **pg_trgm trigram word-similarity** (`word_similarity(q,name) > 0.3` OR substring fallback, ranked best-first) so recognized/typed names match stored items despite punctuation, word order, and small mishears (e.g. "dope max pre-workout" → "Dope-Max Pre-Workout"). Migration `0003_item_search_trgm.sql` enables `pg_trgm` + a GIN trigram index on `input_item.name`. Improves both the recognize auto-match and the confirm-card search. Integration test + browser-verified. |
