...
> **Status:** Living document. **Last updated:** 2026-06-15 (v2-2f: Log capture overhaul — photo / voice / recent, recognize + match + quick-confirm)
...
| 2026-06-15 | Slice v2-2f (Log capture overhaul, in review): added R-CAP-18/19 + ADR-020; R-CAP-16 → Built. The Log screen replaces the freeform manual form with **photo**, **voice**, and **recent-item** capture. New seams: `IntakeRecognizer` (`recognize.ts`/`recognize_anthropic.ts` — Claude vision/text) and `Transcriber` (`transcribe.ts`/`transcribe_openai.ts` — OpenAI-compatible Whisper). New routes: `POST /api/transcribe`, `POST /api/intake/recognize` (recognizes + `ilike`-matches the catalog), `GET /api/intake/recent-items`. The client recognizes → quick-confirm → log against a match / **save as new item** / log by name. AI seams optional (503 + graceful UI). Server tests (pure parsers + routes with mocked seams) + web client tests; recent→confirm→log + 503 fallbacks browser-verified; live photo/voice device-verified. |
