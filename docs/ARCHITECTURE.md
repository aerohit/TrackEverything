...
- **Secrets, never committed:** real values live in Deno Deploy env vars (runtime, per env), GitHub
  ...
  is tracked.
- **Automated migrations:** `migrate.yml` runs `db/migrate.ts` against the pushed branch's environment
...
**Status:** Accepted (2026-06-19). Realizes R-CAP-32; complements [ADR-039](#adr-039) (the product/recipe/stack
kinds) and [ADR-030](#adr-030) (the form-per-kind manage screen).
**Context:** A recipe is the composite "a dish made of other items". The manage screen could only create a
recipe via the generic item form, which added **substance** components (not product members) — so recipes
and products were indistinguishable in practice, and there was no way to build a recipe from real catalog
products. The owner wants a recipe to be built from **products that already exist** in the catalog, and only
products.
**Decision:**
- **Form:** the Regular Items screen gains a third form, **Create recipe** (`ItemDraftForm` `mode="recipe"`):
  a serving + a **product** member picker (`childItemId` components). The member list/autocomplete is
  `eligibleMembers(items, "recipe")` = `kind === 'product'`; a typed name must resolve to a catalog product
  or it's flagged and dropped on save. The lists split into *Your Regular Items* (products), *Your Recipes*,
  *Your Stacks*. The item form loses its kind dropdown — it now makes a **product** only (recipes have their
  own form; stacks already did).
- **Server rule:** `createItem` rejects (400) a recipe whose `childItemId` members aren't existing,
  non-deleted `product` items, validating **before** any insert so a bad member can't half-create an item.
  (Stacks keep accepting any non-stack member.)
**Consequences:** Recipes are now genuinely product-composed and enforced both client- and server-side.
Resolution is unchanged (a recipe scales its child components per serving). The old "recipe with substance
components" shape is no longer creatable from the UI (existing such rows still resolve).
